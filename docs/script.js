...
fetch('data/catalog.json')
  ...
  .then(data => {
    repoData = data;
    sortByStars(); // 初期ソート（Stars 降順）
  })
  .catch(err => {
    console.error(err);
    const tbody = document.querySelector('#catalog tbody');
    tbody.innerHTML = `<tr><td colspan="12">データの読み込みに失敗しました</td></tr>`;
  });
...
function sortByStars() {
  repoData.sort((a, b) => {
    return starSortDesc ? (b.stars || 0) - (a.stars || 0) : (a.stars || 0) - (b.stars || 0);
  });
